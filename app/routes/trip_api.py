...
def get_stops(trip_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    return jsonify(TripService.get_trip_stops(trip_id)), 200
...
def update_stop(trip_id, stop_id):
    ...
    stop = TripService.update_stop(stop_id, trip_id, **data)
    if not stop: return {'error': 'Stop not found'}, 404
...
def delete_stop(trip_id, stop_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    if TripService.delete_stop(stop_id, trip_id):
...
def get_activities(trip_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    return jsonify(TripService.get_trip_activities(trip_id)), 200
...
def update_activity(trip_id, act_id):
    ...
    act = TripService.update_activity(act_id, trip_id, **data)
    if not act: return {'error': 'Activity not found'}, 404
...
def delete_activity(trip_id, act_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    if TripService.delete_activity(act_id, trip_id):
...
def reorder_activities(trip_id):
    ...
    TripService.reorder_activities(trip_id, activity_ids)
...
def get_budget(trip_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    return jsonify(TripService.get_trip_budget_items(trip_id)), 200
...
def add_budget(trip_id):
    ...
    item = TripService.add_budget_item(
        trip_id, data['category'], data.get('description', ''),
...
def manage_budget(trip_id, item_id):
    ...
    if request.method == 'DELETE':
        success = TripService.delete_budget_item(item_id, trip_id)
        return ({'message': 'Deleted'}, 200) if success else ({'error': 'Not found'}, 404)
    data = request.get_json()
    item = TripService.update_budget_item(item_id, trip_id, **data)
...
def manage_packing(trip_id):
    ...
    if request.method == 'GET':
        return jsonify(TripService.get_trip_packing_items(trip_id)), 200
    data = request.get_json()
    item = TripService.add_packing_item(trip_id, data['item_name'], data.get('category', 'General'))
    return jsonify(item), 201
...
def modify_packing(trip_id, item_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    if request.method == 'DELETE':
        success = TripService.delete_packing_item(item_id, trip_id)
        return ({'message': 'Deleted'}, 200) if success else ({'error': 'Not found'}, 404)
    data = request.get_json()
    item = TripService.toggle_packing_item(item_id, trip_id, data.get('is_packed', False))
    return (jsonify(item), 200) if item else ({'error': 'Not found'}, 404)
...
def manage_notes(trip_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    if request.method == 'GET':
        return jsonify(TripService.get_trip_notes(trip_id)), 200
    data = request.get_json()
    note = TripService.add_note(trip_id, data['content'], data.get('title', ''))
...
def modify_notes(trip_id, note_id):
    if not verify_trip_access(trip_id): return {'error': 'Unauthorized'}, 403
    if request.method == 'DELETE':
        success = TripService.delete_note(note_id, trip_id)
        return ({'message': 'Deleted'}, 200) if success else ({'error': 'Not found'}, 404)
    data = request.get_json()
    note = TripService.update_note(note_id, trip_id, data['content'], data.get('title'))
    return (jsonify(note), 200) if note else ({'error': 'Not found'}, 404)
